...
export class IndexedDbRewardClaimRepository implements RewardClaimRepository {
  ...
  async claim(playerId: string, rewardKey: string, now = new Date()): Promise<void> {
    const database = await this.databaseProvider();
    const transaction = database.transaction(['claims', 'progress', 'motivation', 'dailyActivity'], 'readwrite');
    try {
      ...
      const days = await transaction.objectStore('dailyActivity').index('by-player').getAll(playerId);
