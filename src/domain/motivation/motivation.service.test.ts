...
describe('local activity', () => {
  ...
  it('uses local dates across midnight and counts each date once', () => {
    ...
    expect(weeklyActivity([before, before, after].map(date => ({ localDate: localDateKey(date), completedSessions: 5, activeLearningMs: 0 })), after)).toHaveLength(2);
  });
  ...
  it('uses Monday–Sunday weeks including DST transitions', () => {
    ...
    expect(weeklyActivity(['2026-03-22', '2026-03-29', '2026-03-30'].map(localDate => ({ localDate, completedSessions: 5, activeLearningMs: 0 })), new Date(2026, 2, 29, 12))).toEqual(['2026-03-29']);
  });
  it('counts only days with five completed rounds or enough active learning', () => {
    const days = ['2026-09-07', '2026-09-08'].map(localDate => ({ ...createDailyActivity('child', localDate), completedSessions: 5 }));
    expect(calculateWeeklyGoalProgress(days, 3, new Date(2026, 8, 9)).completedDays).toBe(2);
    expect(calculateWeeklyGoalProgress([{ ...days[0]!, completedSessions: 4 }], 3, new Date(2026, 8, 9)).completedDays).toBe(0);
  });
});
