...
export function HomePage() {
  ...
  const motivationError = useMotivationStore((state) => state.error);
  ...
  return (
    <AppShell>
      <section className={styles.page}>
        {motivationError && <p role="alert">{motivationError}<button onClick={() => void refreshMotivation(player.id)}>Erneut laden</button></p>}
        <header className={styles.topbar}>
          <div className={styles.scoreChip}>✨ {progress?.totalScore ?? 0} Punkte</div>
