    ...
    <AppShell><main className={styles.page}>
      ...
      {!ready ? <p>Laden…</p> : <>
        ...
        <section className={styles.panel}><h2>Aktivität von {player.name}</h2>
          ...
          <p>{data.weeklyProgress?.completedDays ?? 0} aktive Tage diese Woche · {data.activities.filter(isActiveDay).length} Trainingstage insgesamt</p>
        ...
        <section className={styles.panel}><h2>Letzte Ergebnisse</h2>
          {!data.sessions.length ? <p>Noch keine abgeschlossene Runde.</p> : <div className={styles.tableWrap}><table className={styles.table}>
            ...
            <tbody>{data.sessions.slice(0, 10).map(session => <tr key={session.id}>
              ...
              <td>{session.correctAnswers} / {session.correctAnswers + session.wrongAnswers}</td><td>{session.masteryVersion === MASTERY_VERSION ? `+${session.stars} Stern` : 'Früheres Sternesystem'}</td><td>+{session.coinsEarned}</td>
            </tr>)}</tbody>
          </table></div>}
        </section>
